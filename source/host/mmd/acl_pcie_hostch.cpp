...
#if defined (LINUX)
  *result = NULL;
  posix_memalign (result, PAGE_SIZE, size);
#endif  // LINUX
#if defined (WINDOWS)
...
ACL_PCIE_HOSTCH::ACL_PCIE_HOSTCH( WDC_DEVICE_HANDLE dev, ACL_PCIE_MM_IO_MGR *io, ACL_PCIE_DEVICE *pcie, ACL_PCIE_DMA *dma ) {
...
#endif  // LINUX
#if defined (WINDOWS)
      m_dma->hostch_destroy(ACL_HOST_CHANNEL_1);
#endif  // WINDOWS
...
#endif // LINUX
#if defined (WINDOWS)
      m_dma->hostch_destroy(ACL_HOST_CHANNEL_0);
#endif // WINDOWS
...
unsigned int ACL_PCIE_HOSTCH::get_hostch_version()
{
  ...
  // If BSP version ID is from prior to host channels,
  // return host channel version as a10gx's
  if (version <= (unsigned int)ACL_VERSIONID_COMPATIBLE_171a) {
...
void ACL_PCIE_HOSTCH::destroy_sync_thread()
{
  if (m_sync_thread_valid) {
    if (m_sync_thread != NULL)
      acl_aligned_free(m_sync_thread);
...
int ACL_PCIE_HOSTCH::create_hostchannel(char * name, size_t queue_depth, int direction)
{
   ...
   ACL_PCIE_DEBUG_MSG_VERBOSE(VERBOSITY_BLOCKTX, ":::: [HOST CHANNEL] Host Channel version read was %u\n", hostch_version);
   
   // Check if channel name user wants to open exists
   if ((strlen(name) == strlen(ACL_HOST_CHANNEL_0_NAME)) &&
       (strncmp(ACL_HOST_CHANNEL_0_NAME, name, strlen(ACL_HOST_CHANNEL_0_NAME)) == 0))
   {
      // Check if hostchannel version is one that has ACL_HOST_CHANNEL_0
      if (hostch_version != ACL_HOSTCH_TWO_CHANNELS) {
         ACL_PCIE_DEBUG_MSG_VERBOSE(VERBOSITY_BLOCKTX, ":::: [HOST CHANNEL] Host Channel %s does not exist in currently programmed device.\n",
              ACL_HOST_CHANNEL_0_NAME);
         return ERROR_INVALID_CHANNEL;
      }

      // check if the direction for the channel is correct
      if (direction != ACL_HOST_CHANNEL_0_WRITE)
         return ERROR_INCORRECT_DIRECTION;
      
      // Check if channel was already opened previously
      if (m_hostch_push_valid) {
         ACL_PCIE_DEBUG_MSG_VERBOSE(VERBOSITY_BLOCKTX, ":::: [HOST CHANNEL] Host Channel '%s' already open\n", ACL_HOST_CHANNEL_0_NAME);
         return ERROR_CHANNEL_PREVIOUSLY_OPENED;
      }
      
      // Make sure the channel depth is at most 1MB, power-of-2, and divisible by page_size
      size_t queue_depth_upper_pow2 = pow(2, ceil(log((double) queue_depth)/log(2.)));
      size_t channel_depth = (queue_depth_upper_pow2 >= HOSTCH_MAX_BUF_SIZE) ? HOSTCH_MAX_BUF_SIZE :
          queue_depth_upper_pow2&(HOSTCH_MAX_BUF_SIZE-PAGE_SIZE);
      
      // Make sure the channel depth is at least 4KB
      if (!channel_depth)
         channel_depth = PAGE_SIZE;
...
#endif  // LINUX
#if defined (WINDOWS)
      m_dma->hostch_create(m_push_queue, m_push_queue_pointer, channel_depth, ACL_HOST_CHANNEL_1);
#endif  // WINDOWS
   
      m_hostch_push_valid = 1;
      return ACL_HOST_CHANNEL_0_ID;
   } else if ((strlen(name) == strlen(ACL_HOST_CHANNEL_1_NAME)) &&
       (strncmp(ACL_HOST_CHANNEL_1_NAME, name, strlen(ACL_HOST_CHANNEL_1_NAME)) == 0))
   {

      // Check if hostchannel version is one that has ACL_HOST_CHANNEL_1
      if (hostch_version != ACL_HOSTCH_TWO_CHANNELS) {
         ACL_PCIE_DEBUG_MSG_VERBOSE(VERBOSITY_BLOCKTX, ":::: [HOST CHANNEL] Host Channel %s does not exist in currently programmed device.\n",
      ...
      if (direction != ACL_HOST_CHANNEL_1_WRITE)
         return ERROR_INCORRECT_DIRECTION;
...
#endif  // LINUX
#if defined (WINDOWS)
      m_dma->hostch_create(m_pull_queue, m_pull_queue_pointer, channel_depth, ACL_HOST_CHANNEL_0);
#endif  // WINDOWS

      m_hostch_pull_valid = 1;
      return ACL_HOST_CHANNEL_1_ID;
   } else
   {
...
int ACL_PCIE_HOSTCH::destroy_hostchannel(int channel)
{
   if (channel == ACL_HOST_CHANNEL_0_ID) {
     if (m_hostch_push_valid) {
       ...
       ACL_PCIE_DEBUG_MSG_VERBOSE(VERBOSITY_BLOCKTX, ":::: [HOST CHANNEL] Host Channel %s is not open.\n", ACL_HOST_CHANNEL_0_NAME);
...
void *ACL_PCIE_HOSTCH::get_buffer( size_t *buffer_size, int channel, int *status)
{
   ...
   if (channel == ACL_HOST_CHANNEL_0_ID)
   {
      ...
      *status = 0;
      ...
      size_t cont_push = (m_push_queue_size > m_push_queue_local_end_p + push_buf_avail) ? push_buf_avail : m_push_queue_size - m_push_queue_local_end_p;
      size_t loop_push = (m_push_queue_size > m_push_queue_local_end_p + push_buf_avail) ? 0 : (m_push_queue_local_end_p + push_buf_avail - m_push_queue_size);
   ...
   } else if (channel == ACL_HOST_CHANNEL_1_ID)
   {
      ...
      *status = 0;
      ...
      size_t cont_pull = (m_pull_queue_size > m_pull_queue_local_front_p + m_pull_queue_available) ? m_pull_queue_available : (m_pull_queue_size - m_pull_queue_local_front_p);
      size_t loop_pull = (m_pull_queue_size > m_pull_queue_local_front_p + m_pull_queue_available) ? 0 : (m_pull_queue_local_front_p + m_pull_queue_available - m_pull_queue_size);
...
size_t ACL_PCIE_HOSTCH::ack_buffer( size_t send_size , int channel, int *status)
{
   if (channel == ACL_HOST_CHANNEL_0_ID)
   {
      ...
      *status = 0;
      ...
      size_t cont_push = (m_push_queue_size > m_push_queue_local_end_p + current_push) ? current_push : (m_push_queue_size - m_push_queue_local_end_p);
      
      // Update the end index that the driver thread will read, to write the update to host channel IP
      // and loop around
      m_push_queue_local_end_p = (m_push_queue_local_end_p + current_push >= m_push_queue_size ) ? 0 : m_push_queue_local_end_p + current_push;
   ...
   } else if(channel == ACL_HOST_CHANNEL_1_ID)
   {
      ...
      *status = 0;
      ...
      size_t cont_pull = (m_pull_queue_size > m_pull_queue_local_front_p + driver_pulled) ? driver_pulled : (m_pull_queue_size - m_pull_queue_local_front_p);
      
      // Update the front index that the driver thread will read, to write the update to host channel IP
      // and loop around
      m_pull_queue_local_front_p = (m_pull_queue_local_front_p + driver_pulled >= m_pull_queue_size ) ? 0 : m_pull_queue_local_front_p + driver_pulled;
